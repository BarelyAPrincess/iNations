...
/**
 * Represents one end (client or server) of a single WebSocketImpl connection. Takes care of the "handshake" phase, then
...
public class WebSocketImpl implements WebSocket
{
	...
	public static/* final */boolean DEBUG = false; // must be final in the future in order to take advantage of VM
																	// optimization
	...
	{
		if ( listener == null || ( draft == null && role == Role.SERVER ) )// socket can be null because we want do be
																									// able to create the object without already
																									// having a bound channel
	 ...
	 *           <code>remote</code> may also be true if this endpoint started the closing handshake since the other
	 *           endpoint may not simply echo the <code>code</code> but close the connection the same time this endpoint
